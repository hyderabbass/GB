...
//			if(PRODUCT_SERIAL) { echo '<td><input class="span2 tran2" name="serial\'+ count +\'" type="text" value=""></td>'; } 
	...
	<?php } ?> 
		...
		$('#payment_type_l').on('click', function(){ setTimeout(function(){ $('#payment_type_s').trigger('liszt:open'); }, 0); });
        $('#deliver_type_l').on('click', function(){ setTimeout(function(){ $('#deliver_type_s').trigger('liszt:open'); }, 0); });
        $('#due_date_l').on('click', function(){ setTimeout(function(){ $('#due_date_s').trigger('liszt:open'); }, 0); });
        $('#order_type_l').on('click', function(){ setTimeout(function(){ $('#order_type_s').trigger('liszt:open'); }, 0); });
...
<div class="control-group">
  <label class="control-label" for="coordinator_name"><?php echo $this->lang->line("coordinator_name"); ?></label>
  <div class="controls"> <?php echo form_input($coordinator_name, (isset($_POST['coordinator_name']) ? $_POST['coordinator_name'] : ""), 'class="span4" id="coordinator_name" required="required" data-error="'.$this->lang->line("coordinator_name").' '.$this->lang->line("is_required").'"');?></div>
</div>
<div class="control-group">
  ...
  <div class="controls">  <?php 
	 $pl[""] = "";
	   		foreach($payment_types as $payment_type){
				$pl[$payment_type->id] = $payment_type->name;
			}
		echo form_dropdown('payment_type', $pl, (isset($_POST['payment_type']) ? $_POST['payment_type'] : ""), 'id="payment_type_s" data-placeholder="'.$this->lang->line("select").' '.$this->lang->line("payment_type").'" required="required" data-error="'.$this->lang->line("payment_type").' '.$this->lang->line("is_required").'"'); ?> </div>
</div>

<div class="control-group">
  <label class="control-label" id="deliver_type_l"><?php echo $this->lang->line("deliver_type"); ?></label>
  <div class="controls">  <?php 
	 $pls[""] = "";
	   		foreach($deliver_types as $deliver_type){
				$pls[$deliver_type->id] = $deliver_type->name;
			}
		echo form_dropdown('deliver_type', $pls, (isset($_POST['deliver_type']) ? $_POST['deliver_type'] : ""), 'id="deliver_type_s" data-placeholder="'.$this->lang->line("select").' '.$this->lang->line("deliver_type").'" required="required" data-error="'.$this->lang->line("deliver_type").' '.$this->lang->line("is_required").'"'); ?> </div>
</div>

<div class="control-group">
  <label class="control-label" id="due_date_l"><?php echo $this->lang->line("due_date"); ?></label>
  <div class="controls">  <?php 
	 $dd[''] = '';
	  foreach($due_dates as $due_date){
    		$dd[$due_date->id] = $due_date->name;
		}
		echo form_dropdown('due_date', $dd, (isset($_POST['due_date']) ? $_POST['due_date'] : DEFAULT_DUE_DATE), 'id="due_date_s" data-placeholder="'.$this->lang->line("select").' '.$this->lang->line("due_date").'" required="required" data-error="'.$this->lang->line("due_date").' '.$this->lang->line("is_required").'"'); ?> </div>
</div>
<div class="control-group">
  <label class="control-label" for="driver_name"><?php echo $this->lang->line("driver_name"); ?></label>
  <div class="controls"> <?php echo form_input($driver_name, (isset($_POST['driver_name']) ? $_POST['driver_name'] : ""), 'class="span4" id="driver_name" required="required" data-error="'.$this->lang->line("driver_name").' '.$this->lang->line("is_required").'"');?></div>
</div>
<div class="control-group">
  <label class="control-label" id="order_type_l"><?php echo $this->lang->line("order_type"); ?></label>
  <div class="controls">  <?php 
	 $plt[""] = "";
	   		foreach($order_types as $order_type){
				$plt[$order_type->id] = $order_type->name;
			}
		echo form_dropdown('order_type', $plt, (isset($_POST['order_type']) ? $_POST['order_type'] : ""), 'id="order_type_s" data-placeholder="'.$this->lang->line("select").' '.$this->lang->line("order_type").'" required="required" data-error="'.$this->lang->line("order_type").' '.$this->lang->line("is_required").'"'); ?> </div>
</div>
<div class="control-group">
  <label class="control-label" for="served_by"><?php echo $this->lang->line("served_by"); ?></label>
  <div class="controls"> <?php echo form_input($served_by, (isset($_POST['served_by']) ? $_POST['served_by'] : ""), 'class="span4" id="served_by" "');?></div>
</div>
<!--<div class="control-group">
  <label class="control-label" for="payment_type_l"><?php echo $this->lang->line("payment_type"); ?></label>
  <div class="controls">
      <div class="btn-group payment_type" data-toggle="buttons-radio">
    <button type="button" value="1" class="btn"><?php echo $this->lang->line("cash"); ?></button>
    <button type="button" value="2" class="btn"><?php echo $this->lang->line("cheque"); ?></button>
    <button type="button" value="3" class="btn"><?php echo $this->lang->line("credit"); ?></button>
    </div>
    <input type="hidden" name="payment_type" id="payment_type" value="<?php /* echo $payment_type->id; */ ?>">
  </div>
</div>-->
