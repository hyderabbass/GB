...
//index or inventories page
   ...
   function getdatatableajax()
   {
	   ...
	   $this->datatables
			...
			"<center><a href='#' title='$2' class='tip' data-html='true'><i class='icon-folder-close'></i></a> <a target='_blank' href='index.php?module=pos&view=view_invoice&id=$1' title='".$this->lang->line("view_invoice")."' class='tip'><i class='icon-fullscreen'></i></a>
   ...
   function getwhinv($warehouse_id = NULL)
   {
	   ...
	   $this->datatables
			->select("id, date, reference_no, biller_name, payment_type, deliver_type, due_date, order_type, customer_name, total_tax, total_tax2, total")
			->from('sales')
...
//view inventory as html page
   ...
   function view_invoice()
   {
	   ...
	   if($this->input->get('quote_id')){ $quote_id = $this->input->get('quote_id'); } else { $quote_id = NULL; }
           ...
           $data['payment_type'] = $this->sales_model->getPaymentByID($payment_id);
           $data['deliver_type'] = $this->sales_model->getDeliverByID($deliver_id);
           $data['due_date'] = $this->sales_model->getDueByID($due_id);
           $data['order_type'] = $this->sales_model->getOrderByID($order_id);
...
//Add new invoice
   ...
   function add()
   {
	   $groups = array('purchaser', 'viewer');
        ...
        $this->form_validation->set_rules('driver_name', $this->lang->line("driver_name"), 'required|xss_clean');
        ...
        $this->form_validation->set_rules('deliver_type', $this->lang->line("deliver_type"), 'required|is_natural_no_zero|xss_clean');
         $this->form_validation->set_rules('due_date', $this->lang->line("due_date"), 'required|is_natural_no_zero|xss_clean');
		...
		if ($this->form_validation->run() == true)
		{
            ...
            $payment_details = $this->sales_model->getPaymentByID($payment_id);
			$payment_type = $payment_details->name;
            $deliver_id = $this->input->post('deliver_type');
            $deliver_details = $this->sales_model->getDeliverByID($deliver_id);
			$deliver_type = $deliver_details->name;
            $due_id = $this->input->post('due_date');
            $due_details = $this->sales_model->getDueByID($due_id);
			$due_date = $due_details->name;
            $order_id = $this->input->post('order_type');
            $order_details = $this->sales_model->getOrderByID($order_id);
		...
		{ 
		  $sale_id  = $this->sales_model->addSale($saleDetails, $items, $warehouse_id);
        
			if($sale_id != false) {	
			 
				$this->session->set_flashdata('success_message', $this->lang->line("sale_added"));
                
				redirect("module=sales&view=view_invoice&id=$sale_id", 'refresh');
			}
		...
		{  
            ...
            $data['driver_name'] = array('name' => 'driver_name',
				'id' => 'driver_name',
				'type' => 'text',
				'value' => $this->form_validation->set_value('driver_name'),
			);
            $data['served_by'] = array('name' => 'served_by',
				'id' => 'served_by',
				'type' => 'text',
				'value' => $this->form_validation->set_value('served_by'),
			);
            ...
            $data['due_date'] = array('name' => 'due_date',
				'id' => 'due_date',
				'type' => 'select',
				'value' => $this->form_validation->set_select('due_date'),
			);
       ...
       $data['payment_types'] = $this->sales_model->getAllPayments();
       $data['deliver_types'] = $this->sales_model->getAllDelivers();
       $data['due_dates'] = $this->sales_model->getAllDues();
       $data['order_types'] = $this->sales_model->getAllOrders();
...
function quote_to_invoice()
    {
	   ...
	   $groups = array('purchaser', 'viewer');
		if ($this->ion_auth->in_group($groups))
		{
		  ...
		  $data['billers'] = $this->sales_model->getAllBillers();
          $data['payment_types'] = $this->sales_model->getAllPayments();
          $data['deliver_types'] = $this->sales_model->getAllDeliveries();
		  $data['customers'] = $this->sales_model->getAllCustomers();
		  $data['warehouses'] = $this->sales_model->getAllWarehouses();
		  $data['tax_rates'] = $this->sales_model->getAllTaxRates();
		  $data['discounts'] = $this->sales_model->getAllDiscounts();		
   ...
   function getdeliveries()
   {
	   ...
	   $this->load->library('datatables');
	   $this->datatables
			->select("id, date, time, coordinator_name, driver_name, reference_no, customer, address")
			->from('deliveries');
			$this->datatables->add_column("Actions", 
