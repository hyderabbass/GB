...
class Categories_model extends CI_Model
{
	...
	{
		$q = $this->db->get_where("categories", array('menu_id' => $category_id));
		if($q->num_rows() > 0) {
			foreach (($q->result()) as $row) {
				$data[] = $row;
			}
	...
	public function getCategoryByID($id) 
	{
		...
		$q = $this->db->get_where("menus", array('id' => $id), 1); 
		  if( $q->num_rows() > 0 )
		  {
			return $q->row();
		  } 
	...
	public function addCategory($name, $code)
	{
		...
		if($this->db->insert("menus", array('code' => $code, 'name' => $name))) {
			return true;
		} else {
			return false;
		}
	...
	public function addSubCategory($category, $name, $code)
	{
		...
		if($this->db->insert("categories", array('menu_id' => $category,'code' => $code, 'name' => $name))) {
			return true;
		} else {
			return false;
		}
	...
	public function updateCategory($id, $data = array())
	{
		...
		$this->db->where('id', $id);
		if($this->db->update("menus", $categoryData)) {
			return true;
		} else {
			return false;
		}
