...
*/
	...
	function edit($id = NULL)
	{
		...
		$pr_details = $this->categories_model->getCategoryByID($id);
			if ($this->input->post('code') != $pr_details->code) {
				$this->form_validation->set_rules('code', $this->lang->line("category_code"), 'is_unique[menus.code]');
			}
   ...
   function getsubcategory()
   {
	   ...
	   $this->load->library('datatables');
	   $this->datatables
			->select("categories.id as id, categories.code as scode, categories.name as sname, menus.name as cname")
			->from("categories")
			->join('menus', 'menus.id = categories.menu_id', 'left')
	...
	function add_subcategory()
	{
		...
		$this->form_validation->set_rules('code', $this->lang->line("subcategory_code"), 'trim|is_unique[menus.code]|is_unique[categories.code]|required|xss_clean');
		...
		$data['categories'] = $this->categories_model->getAllMenus();
