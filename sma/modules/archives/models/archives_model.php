...
class Archives_model extends CI_Model
{
	...
	{
		$this->db->select("reference_no, warehouse_id, biller_id, biller_name, payment_id, payment_type, deliver_id, deliver_type, order_id, order_type, customer_id, customer_name, date, note, inv_total, total_tax, total");
		$this->db->from('quotes');
		$this->db->where('id', $id);
		$this->db->limit(1);
		$q = $this->db->get(); 
	...
	public function addSale($saleDetails = array(), $items = array(), $warehouse_id)
	{
		...
		$saleData = array(
            ...
            'payment_type'			=> $saleDetails['payment_type'],
