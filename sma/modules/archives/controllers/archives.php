...
//index or inventories page
   ...
   function index()
   {
       ...
       $data['warehouses'] = $this->archives_model->getAllWarehouses();
      $meta['page_title'] = $this->lang->line("archives");
	  $data['page_title'] = $this->lang->line("archives");
   ...
   function getdatatableajax()
   {
	   ...
	   $this->datatables
			->select("sales.id as sid, date, reference_no, biller_name, customer_name, status, total_tax2, total, internal_note")
			->from('sales')
            ->where($data);
			...
			<a href='index.php?module=archives&amp;view=delete&amp;id=$1' onClick=\"return confirm('". $this->lang->line('alert_y_invoice') ."')\" title='".$this->lang->line("restore_invoice")."' class='tip'><i class='icon-envelope'></i></a></center>", "sid, internal_note")
   ...
   function getwhinv($warehouse_id = NULL)
   {
	   ...
	   $this->datatables
			->select("id, date, reference_no, biller_name, payment_type, deliver_type, order_type, customer_name, total_tax, total_tax2, total")
			->from('sales')
   ...
   function total()
   {
	  ...
	  $data['monthly_sales'] = $this->archives_model->getmonthlySales();
	  $data['monthly_purchases'] = $this->archives_model->getmonthlyPurchases();
...
//view inventory as html page
   ...
   function view_invoice()
   {
	   ...
	   if($this->input->get('quote_id')){ $quote_id = $this->input->get('quote_id'); } else { $quote_id = NULL; }
	   	   ...
	   	   $data['message'] = (validation_errors() ? validation_errors() : $this->session->flashdata('message'));
		   $data['rows'] = $this->archives_model->getAllInvoiceItems($sale_id);
		   
		   $inv = $this->archives_model->getInvoiceBySaleID($sale_id);
		   $biller_id = $inv->biller_id;
           $payment_id = $inv->payment_id;
           $deliver_id = $inv->deliver_id;
           $order_id = $inv->order_id;
		   $customer_id = $inv->customer_id;
		   $invoice_type_id = $inv->invoice_type;
		   $data['biller'] = $this->archives_model->getBillerByID($biller_id);
           $data['payment_type'] = $this->archives_model->getPaymentByID($payment_id);
           $data['deliver_type'] = $this->archives_model->getDeliverByID($deliver_id);
           $data['order_type'] = $this->archives_model->getOrderByID($order_id);
		   $data['customer'] = $this->archives_model->getCustomerByID($customer_id);
		   $data['invoice_types_details'] = $this->archives_model->getInvoiceTypeByID($invoice_type_id);
   ...
   function email_invoice()
   {
	   ...
	   if($this->input->get('quote_id')){ $quote_id = $this->input->get('quote_id'); } else { $quote_id = NULL; }
		...
		$data['from_email'] = $user->email;
			...
			$inv = $this->archives_model->getInvoiceByID($id);
			$customer_id = $inv->customer_id;
		
			$data['cus'] = $this->archives_model->getCustomerByID($customer_id);
			$data['id'] = $id;
			$data['quote_id'] = NULL;
...
//Add new invoice
   ...
   function add()
   {
	   $groups = array('purchaser', 'viewer');
		...
		$this->form_validation->set_rules('payment_type', $this->lang->line("payment_type"), 'required|is_natural_no_zero|xss_clean');
        $this->form_validation->set_rules('deliver_type', $this->lang->line("deliver_type"), 'required|is_natural_no_zero|xss_clean');
        $this->form_validation->set_rules('order_type', $this->lang->line("order_type"), 'required|is_natural_no_zero|xss_clean');
		...
		if ($this->form_validation->run() == true)
		{
			...
			$biller_details = $this->archives_model->getBillerByID($biller_id);
			$biller_name = $biller_details->name;
            $payment_id = $this->input->post('payment_type');
            $payment_details = $this->archives_model->getPaymentByID($payment_id);
			$payment_type = $payment_details->name;
            $deliver_id = $this->input->post('deliver_type');
            $deliver_details = $this->archives_model->getDeliverByID($deliver_id);
			$deliver_type = $deliver_details->name;
            $order_id = $this->input->post('order_type');
            $order_details = $this->archives_model->getOrderByID($order_id);
			$order_type = $order_details->name;
			$customer_id = $this->input->post('customer');
			$customer_details = $this->archives_model->getCustomerByID($customer_id);
				...
				for($i=1; $i<=500; $i++){
					if( $this->input->post($quantity.$i) && $this->input->post($product.$i) && $this->input->post($unit_price.$i) ) {
						...
						if(TAX1) { 
							...
							$tax_details = $this->archives_model->getTaxRateByID($tax_id);
						...
						if(DISCOUNT_METHOD == 1 && DISCOUNT_OPTION == 2) {
							...
							$ds_details = $this->archives_model->getDiscountByID($discount_id);
			...
			if(!empty($inv_product_code)) {	 
				foreach($inv_product_code as $pr_code){
					$product_details = $this->archives_model->getProductByCode($pr_code);
			...
			if(TAX2) {
				$tax_dts = $this->archives_model->getTaxRateByID($tax_rate2);
				$taxRt = $tax_dts->rate;
				$taxTp = $tax_dts->type;	
			...
			if(DISCOUNT_METHOD == 1 && DISCOUNT_OPTION == 1) {
				...
				$ds_dts = $this->archives_model->getDiscountByID($inv_discount);
				$ds = $ds_dts->discount;
				$dsTp = $ds_dts->type;	
		...
		{ 
			if($this->archives_model->addSale($saleDetails, $items, $warehouse_id)) {		
		...
		{  
            ...
            $data['deliver_type'] = array('name' => 'deliver_type',
				'id' => 'deliver_type',
				'type' => 'select',
				'value' => $this->form_validation->set_select('deliver_type'),
			);
            $data['order_type'] = array('name' => 'order_type',
				'id' => 'order_type',
				'type' => 'select',
				'value' => $this->form_validation->set_select('order_type'),
			);
	   ...
	   $data['billers'] = $this->archives_model->getAllBillers();
       $data['payment_types'] = $this->archives_model->getAllPayments();
       $data['deliver_types'] = $this->archives_model->getAllDelivers();
       $data['order_types'] = $this->archives_model->getAllOrders();
	   $data['customers'] = $this->archives_model->getAllCustomers();
	   $data['warehouses'] = $this->archives_model->getAllWarehouses();
	   $data['products'] = $this->archives_model->getAllProducts();
	   $data['tax_rates'] = $this->archives_model->getAllTaxRates();
	   $data['discounts'] = $this->archives_model->getAllDiscounts();
	   $data['rnumber'] = $this->archives_model->getNextAI();
	   if(DISCOUNT_OPTION == 1) { 
			$discount_details = $this->archives_model->getDiscountByID(DEFAULT_DISCOUNT);
	  ...
	  if(DISCOUNT_OPTION == 2) { 
			$discount2_details = $this->archives_model->getDiscountByID(DEFAULT_DISCOUNT);
	  		$data['discount_rate2'] = $discount2_details->discount;
 			$data['discount_type2'] = $discount2_details->type;
	  } 
	  if(TAX1) {
	  $tax_rate_details = $this->archives_model->getTaxRateByID(DEFAULT_TAX);
	  ...
	  if(TAX2) {
		$tax_rate2_details = $this->archives_model->getTaxRateByID(DEFAULT_TAX2);
   ...
   function edit()
   {
	   ...
	   $groups = array('purchaser', 'viewer');
		...
		if ($this->form_validation->run() == true)
		{
			...
			$note = $this->ion_auth->clear_tags($this->input->post('note'));
			$internal_note = $this->ion_auth->clear_tags($this->input->post('internal_note'));
			$in_voice = $this->archives_model->getInvoiceByID($id); 
		...
		if ( $this->form_validation->run() == true && !empty($items)  )
		{ 
			if($this->archives_model->updateSale($id, $saleDetails, $items, $warehouse_id)) {		
	  ...
	  if(TAX2) {
		...
		$data['tax_name2'] = $tax_rate2_details->name;
 		$data['tax_type2'] = $tax_rate2_details->type;
		  ...
		  $data['tax_rates'] = $this->archives_model->getAllTaxRates();
		  $data['discounts'] = $this->archives_model->getAllDiscounts();				  
		  $data['inv'] = $this->archives_model->getInvoiceByID($id);
		  $data['inv_products'] =  $this->archives_model->getAllInvoiceItems($id);
		  $data['id'] = $id;
...
function delete($id = NULL)
    {
        ...
        if ($this->ion_auth->in_group($groups))
        {
            $this->session->set_flashdata('message', $this->lang->line("access_denied"));
            $data['message'] = (validation_errors() ? validation_errors() : $this->session->flashdata('message'));
            redirect('module=archives', 'refresh');
        }
		
        if($this->input->get('id')){ $id = $this->input->get('id'); } else { $id = NULL; }
		

			if ( $this->archives_model->deleteInvoice($id) )
			{ 
				$this->session->set_flashdata('success_message', $this->lang->line("invoice_restored"));
				redirect('module=archives', 'refresh');
			} 
...
function quote_to_invoice()
    {
	   ...
	   $groups = array('purchaser', 'viewer');
		if ($this->ion_auth->in_group($groups))
		{
		  ...
		  $data['billers'] = $this->archives_model->getAllBillers();
          $data['payment_types'] = $this->archives_model->getAllPayments();
          $data['deliver_types'] = $this->archives_model->getAllDeliveries();
		  $data['customers'] = $this->archives_model->getAllCustomers();
		  $data['warehouses'] = $this->archives_model->getAllWarehouses();
		  $data['tax_rates'] = $this->archives_model->getAllTaxRates();
		  $data['discounts'] = $this->archives_model->getAllDiscounts();		

		  $data['inv'] = $this->archives_model->getQuoteByID($quote_id);
		  $data['inv_products'] =  $this->archives_model->getAllQuoteItems($quote_id);
	  	  $data['invoice_types'] = $this->archives_model->getAllInvoiceTypesFor();
		  $data['id'] = NULL;
   ...
   function getdeliveries()
   {
	   ...
	   $this->load->library('datatables');
	   $this->datatables
			->select("id, date, time, coordinator_name, reference_no, customer, address")
			->from('deliveries');
			$this->datatables->add_column("Actions", 
   ...
   function view_delivery()
   {
	  ...
	  $data['delivery'] =  $this->archives_model->getDeliveryByID($id);
	...
	function add_delivery()
   {
	   ...
	   $groups = array('purchaser', 'viewer');
		...
		if ( $this->form_validation->run() == true && $this->archives_model->addDelivery($dlDetails) )
		{ 
	  ...
	  $sale = $this->archives_model->getInvoiceByID($id);
	  $data['customer'] =  $this->archives_model->getCustomerByID($sale->customer_id);
	  $data['inv'] = $sale;
   ...
   function edit_delivery()
   {
	   ...
	   $groups = array('purchaser', 'viewer');
		...
		if ( $this->form_validation->run() == true && $this->archives_model->updateDelivery($id, $dlDetails) )
		{ 
   ...
   function delete_delivery($id = NULL)
    {
        ...
        if($this->input->get('id')){ $id = $this->input->get('id'); } else { $id = NULL; }
			...
			if ( $this->archives_model->deleteDelivery($id) )
			{ 
	...
	function scan_item()
   {
	   ...
	   if($item = $this->archives_model->getProductByCode($code)) {
    ...
    function add_item()
   {
	   ...
	   if($item = $this->archives_model->getProductByName($name)) {
   ...
   function suggestions()
	{
		...
		$rows = $this->archives_model->getProductNames($term);
	...
	function tax_rates()
   {
	   ...
	   $new_tax_rate_details = $this->archives_model->getTaxRateByID($id);
	   
	   if($old_id) {
		   $old_tax_rate_details = $this->archives_model->getTaxRateByID($old_id);
   ...
   function discounts()
   {
	   if($this->input->get('id')) { $id = $this->input->get('id'); }
	   if($this->input->get('old_id')) { $old_id = $this->input->get('old_id'); } else { $old_id = NULL; }
	   $new_discount_details = $this->archives_model->getDiscountByID($id);
	   
	   if($old_id) {
		   $old_discount_details = $this->archives_model->getDiscountByID($old_id);
