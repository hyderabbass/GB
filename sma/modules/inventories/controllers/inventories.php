    ...
    function add()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($i = 1; $i <= TOTAL_ROWS; $i++) {
                if ($this->input->post($quantity . $i) && $this->input->post($batch_number . $i) && $this->input->post($expiry_date . $i) && $this->input->post($product . $i) && $this->input->post($unit_cost . $i)) {
                    ...
                    $inv_batch_number[] = $this->input->post($batch_number . $i);
                    $inv_expiry_date[] = convert_to_mysql_date($this->input->post($expiry_date . $i)); // issue #13 - Hyder - convert date to mysql-compatible date format - 30 June 2014
            ...
            $keys = array("product_id", "product_code", "product_name", "quantity", "batch_number", "expiry_date", "unit_price", "gross_total");

            $items = array();
            foreach (array_map(null, $product_id, $product_code, $product_name, $inv_quantity, $inv_batch_number, $inv_expiry_date, $inv_unit_cost, $inv_gross_total) as $key => $value) {
                $items[] = array_combine($keys, $value);
            }

        }

        if ($this->form_validation->run() == true && $this->inventories_model->addPurchase($reference, $date, $supplier_id, $supplier_name, $note, $inv_total, $items, $warehouse_id)) { //check to see if we are creating the biller
...
//Edit inventory
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($i = 1; $i <= TOTAL_ROWS; $i++) {
                if ($this->input->post($quantity . $i) && $this->input->post($batch_number . $i) && $this->input->post($expiry_date . $i) && $this->input->post($product . $i) && $this->input->post($unit_price . $i)) {
                    ...
                    $inv_quantity[] = $this->input->post($quantity . $i);
                    $inv_batch_number[] = $this->input->post($batch_number . $i);
                    $inv_expiry_date[] = $this->input->post($expiry_date . $i);
                    $inv_product_code[] = $this->input->post($product . $i);
            ...
            $items = array();
            foreach (array_map(null, $product_id, $product_code, $product_name, $inv_quantity, $inv_batch_number, $inv_expiry_date, $inv_unit_price, $inv_gross_total) as $key => $value) {
                $items[] = array_combine($keys, $value);
            }
    ...
    function csv_inventory()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) /*if($_FILES['userfile']['size'] > 0)*/ {
                ...
                $keys = array('quantity', 'batch_number', 'expiry_date', 'code', 'unit_price');
                ...
                foreach ($final as $csv_pr) {
                    ...
                    $inv_batch_number[] = $csv_pr['batch_number'];
                    $inv_expiry_date[] = $csv_pr['expiry_date'];
        ...
        if ($this->form_validation->run() == true && $this->inventories_model->addPurchase($reference, $date, $supplier_id, $supplier_name, $note, $inv_total, $items, $warehouse_id)) { //check to see if we are creating the inv
