...
class Auth extends MX_Controller
{
    ...
    function create_user()
    {
        ...
        $this->form_validation->set_rules('id_number', $this->lang->line("id_number"), 'required|xss_clean|min_length[14]|max_length[14]');
        ...
        $this->form_validation->set_rules('phone', $this->lang->line("phone"), 'required|xss_clean|min_length[7]|max_length[16]');
        $this->form_validation->set_rules('mobile', $this->lang->line("mobile"), 'required|xss_clean|min_length[7]|max_length[16]');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $dob = convert_to_mysql_date(trim($this->input->post('dob')));

            /*
             * if(JS_DATE == 'dd-mm-yyyy' || JS_DATE == 'dd/mm/yyyy') {
                $dob = substr($user_dob, -4)."-".substr($user_dob, 3, 2)."-".substr($user_dob, 0, 2);
            } else {
                $dob = substr($user_dob, -4)."-".substr($user_dob, 0, 2)."-".substr($user_dob, 3, 2);
            }
            */
    ...
    function edit_user($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $additional_data = array('first_name' => $this->input->post('first_name'),
                'last_name' => $this->input->post('last_name'),
                'address' => $this->input->post('address'),
                'dob' => convert_to_mysql_date($this->input->post('dob')) , // issue #13 - Hyder - 2014
                'id_number' => $this->input->post('id_number'),
                'mobile' => $this->input->post('mobile'),
                'company' => $this->input->post('company'),
                'phone' => $this->input->post('phone'),
