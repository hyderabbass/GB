	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
                ...
                //Updating promotion table by end date.
                   
           $query = $this->db->query('SELECT product_id, end_date, normal_price FROM promotions WHERE curdate() >= end_date ');
            $status = 'inactive';
            $active = 'active';
            ...
            // $this->db->delete('promotions')->where('product_id', $row['product_id']);;
            ...
            $delete_promotion = array(
             ...
             $this->db->update('promotions', $delete_promotion);
          }  
	...
	public function updateUser($id, $email, $password, $additional_data = array(), $group_id) //need to test email activation
	{
		...
		if($password == NULL) {
			...
			$userData = array(
                ...
                'id_number'   		=> $additional_data['id_number'],
