...
class Pos extends MX_Controller
{
    ...
    */
    ...
    function __construct()
    {
        ...
        parent::__construct();
        //  $this->output->enable_profiler(TRUE); // Hyder - profiler
        ...
        define("DMENU", $pos_setting->default_menu);
    ...
    function index()
    {
      ...
      //  $this->output->enable_profiler(TRUE);
        ...
        if ($this->form_validation->run() == true) {
            ...
            $deliver_type = $this->input->post('rdelivertype');
            $order_by = strtolower($this->input->post('rordertype'));
            $count = $this->input->post('count');
            $count = $count - 1;
...
//var_dump($dis . $i,$_POST);
            ...
            if (!empty($inv_product_code)) {
                foreach ($inv_product_code as $pr_code) {
                    ...
                    $product_itemkit_id[] = $product_details->itemkit_id; // # Issue #1 - Hyder - ajoute item_id dans sa array la pu ki ggn li lot coter.
            ...
            $keys = array("product_id", "product_itemkit_id", "product_code", "product_name", "product_unit", "tax_rate_id", "tax", "quantity", "unit_price", "gross_total", "val_tax", "serial_no", "discount_val", "discount", "discount_id");

            $items = array();
            foreach (array_map(null, $product_id,$product_itemkit_id, $product_code, $product_name, $product_unit, $tax_rate_id, $tax, $inv_quantity, $inv_unit_price, $inv_gross_total, $val_tax, $serial, $val_ds, $discount, $dsID) as $key => $value) {
            ...
            $saleDetails = array('reference_no' => $reference_no,
                ...
                'count' => $count,
                'due_date' => $due_date, // issue #6 - Hyder - 8 July 2014
                'served_by' => USER_NAME, // issue #7- Hyder - 14 July 2014
                'order_type' => $order_by, // issue #7 - Hyder - 14 July 2014
        ...
        if ($this->form_validation->run() == true && !empty($items)) {
            ...
            $data['menu'] = $this->pos_model->getAllMenus();
            ...
            $data['products'] = $this->ajaxproducts(DMENU);
            $data['menus'] = $this->posmenus();
    ...
    function add_row($category_id = NULL, $code = NULL)
    {
        ...
        $row = '<td width="29px" style="text-align:center;"> </td><td width="144px">' . $products->name . '</td><td width="44px" style="text-align:center;">1</td><td width="86px style="text-align:right;"> ' . $products->price . ' </td>';
    ...
    function price($code = NULL)
    {
        ...
        $name = $products->name;
        $code = $products->code;
        $menu_id = $products->menu_id; // added menu_id in json response - Hyder - 15 June 2014 - issue #2 - discount mods
//echo $this->db->last_query();
        $data = array('price' => $price, 'name' => $name, 'code' => $code, 'menu_id' => $menu_id); // added menu_id in json response - Hyder - 15 June 2014 - issue #2 - discount mods
        echo json_encode($data);

    }

    function posmenus($menu_id = NULL)
    {
        ...
        $menus = $this->pos_model->getAllMenus();
        $count = 1;
        $submenus = "";
        ...
        foreach ($menus as $menu) {
            ...
            $id = preg_replace('/\s+/', '', $menu->name);
            $id = strtolower($id);
            //<!-- Abdallah - issue #10 - 04-07-2014  - END -->//
            #################################################################################

            $submenus .= "<li><button id='$menu->name' type=\"button\" value='" . $menu->id . "' class=\"gray main_menu\">
			" . $menu->name . "</button></li>";
            $count++;
    ...
    #################################################################################
    //- Abdallah - issue #10 - 04-07-2014  - START
    function poscategories($category_id, $button_id_val)
    {
        $button_id = $this->input->post('button_id_val');
        ...
        //  die();
        $menus = $this->pos_model->getAllMenus();
        $categories = $this->pos_model->getSubCategory($category_id);
        //var_dump($categories);
        // die();
       // echo $this->db->last_query();
        $count = 1;
        ...
        foreach ($categories as $category) {
            ...
            $cats .= "
                <input type=\"hidden\" value='" . $category->id . "'class=\"btn_main\"/>
                <button id=\"category\" type=\"button\" value='" . $category->id . "' class=\"gray1 $category->id\">" . $category->name . "</button>";
            //<!-- Abdallah - issue #10 - 04-07-2014  - END -->//
            #################################################################################
    ...
    function ajaxproducts($category_id = NULL)
    {
        ...
        foreach ($products as $product) {
            ...
            /*#################################################################################*/
            $subcat = "sub_cat" . $category_id; //Abdallah - issue #10 - 07-07-2014  - END
            /*#################################################################################*/

            //  echo "<hr/>";
            if ($category_id < 10) {
                $category_idx = "0" . ($category_id / 100) * 100; //Abdallah - issue #10 - 07-07-2014
            } else {
                $category_idx = ($category_id / 100) * 100; //Hyder  - issue #10 - 16 June 2014 - #related issue #2
            ########################################################################################################
            # Explication: Mone ajoute sa else condition sa parski si categoryid li plis grand ki 10, variable     #
            # $category_idx ti p vine null. Lerla cki p ariver, kan soizir n produit, so default discount pas ti   #
            # p focus lor bon valeur.                                                                              #
            ########################################################################################################
            }
            ...
            if ($product->image == 'no_image.jpg') {
                $prods .= "
                <button id=\"product-" . $category_idx . $count . "\" type=\"button\" value='" . $product->code . "' class=\"green  $subcat\" ><i><img src=\"assets/uploads/thumbs/default.png\"></i><span><span>" . $product->name . "</span></span></button>";
            } else {
                $prods .= "
                <button id=\"product-" . $category_idx . $count . "\" type=\"button\" value='" . $product->code . "' class=\"green  $subcat\" ><i><img src=\"assets/uploads/thumbs/" . $product->image . "\"></i><span><span>" . $product->name . "</span></span></button>";
            }
...
//view inventory as html page
    ...
    function view_invoice()
    {
        ...
        $data['teamleader'] = $this->pos_model->getCustomerByTeamleader($customer_id); // Hyder - get TL name to display on invoice
    ...
    function settings()
    {
        ...
        if ($this->form_validation->run() == true && $this->pos_model->updateSetting($data)) {
            ...
            $data['menus'] = $this->pos_model->getAllMenus();
