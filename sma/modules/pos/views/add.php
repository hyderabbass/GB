        ...
        <div class="pos">
            ...
            <div id="pos"> <?php echo form_open("module=pos"); ?>
                <div id="leftdiv">
                    ...
                    <input type="hidden" name="rduedate" id="rduedate" style="display: none;" />



                    <input type="hidden" name="rpaidby" id="rpaidby" value="cash" style="display: none;" />
                    <input type="hidden" name="rdelivertype" id="rdelivertype" value="driver" style="display: none;" />
                    <input type="hidden" name="rordertype" id="rordertype" value="direct" style="display: none;" /> <!-- issue #7 - Hyder - 14 June 2014 -->
                    ...
                    <div id="cpinner">
                        <div id="catContainer">
                            <div class="list_carousel">
                                ...
                                <a class="prev" id="prev3" href="#"><span>prev</span></a> <a class="next" id="next3" href="#"><span>next</span></a>
...
$(document).ready(function(){
    ...
    $( ".main_menu" ).click(function() {
        ...
        <!--#################################################################################-->
        <!-- Abdallah - issue #10 - 04-07-2014  - START-->
        //get id value of button being clicked
        var category_id = $(this).val();
        var button_id_val = this.id;

        $('li button').each(function (i) {
            if (this.id == button_id_val) {
        ...
        <!-- Abdallah - issue #10 - 04-07-2014  - END -->
        <!--#################################################################################-->

        $.ajax({
            type: "POST",
            url: "index.php?module=pos&view=poscategories",
            data: 'category_id=' + category_id + '&button_id_val=' + button_id_val, <!-- Abdallah - issue #10 - 04-07-2014  - START-->
            dataType: "html",
            success: function(data) {
                $('#procat').empty();
                var newPrs = $('<div></div>');
                newPrs.html(data);
                newPrs.appendTo("#procat");
    ...
    <!--#################################################################################-->
    <!-- Abdallah - issue #10 - 04-07-2014  - START-->
    $('button[class^="green"]').live("click", function () {
        //$('#category').css("color", "#0099ff" );
        var prodid = $(this).attr("class");
        var val = $('#category').val();

        $('div#procat button').css("color", "");

        var class_name = prodid.split(' ');
        class_name = class_name['2']

        var subid = class_name.replace("sub_cat", "");

        $('button.' + subid).css("color", "#0099ff");
    ...
    function loadProducts() {
        $('button[id^="category"]').live( "click", function() {

            <!--#################################################################################-->
            <!-- Abdallah - issue #10 - 04-07-2014  - START-->

            var cat_name = $('.btn_main').val();
            $('#'+cat_name).css("color", "#0099ff" );
            $('div#procat button').css("color", "" );
    ...
    $("#updateRow").click(function() {
        ...
        old_val = 2;  //$('#tax_rate-'+rw_no).val(); - Hyder - tax rate pu bizin 2 tou les temp. mne hard code li parski des fois li p vine null. - 16 July 2014
    ...
    $('.keyboard').keyboard({
        ...
        beforeClose : function(e, keyboard, el, accepted) {
            ...
            $.ajax({
                ...
                url: "index.php?module=pos&view=discounts",
                data: { id: pr_ds_id },
                dataType: "json", // Yogesh ti tir sa - Hyder - 15 June 2014  -
                success: function(data) {
                    new_discount_rate = parseFloat(data.new_discount);
                    new_discount_type = parseFloat(data.new_discount_type);
                },
...
$("#payment").click(function() {
    ...
    <!-- Issue #7 (2nd part) - Hyder - 30 June 2014  -->
    newPTDiv.html('<div class="well form-horizontal" style="margin-bottom:0;"><div class="control-group" style="font-weight:bold;"><div class="control-label" style="padding-top:0;font-weight:bold;"><?php echo $this->lang->line("total_payable"); ?>:</div><div class="controls"><span style="background: #FFFF99; border-radius:5px; padding: 5px 10px; color: #000;">'+ twt +'</span></div></div> <div class="control-group" style="font-weight:bold;"><div class="control-label" style="padding-top:0;font-weight:bold;"><?php echo $this->lang->line("total_items"); ?>:</div><div class="controls"><span style="background: #FFFF99; border-radius:5px; padding: 5px 10px; color: #000;">'+ count +'</span></div></div> <div class="control-group"><label class="control-label" for="paid_by"><?php echo $this->lang->line("paid_by"); ?></label><div class="controls"><select name="paid_by" id="paid_by"><option value="cash">Cash</option><option value="Credit">Credit</option><option value="CC">Credit Card</option><option value="Switch Maestro">Switch Maestro</option><option value="Commission Deduction">Commission Deduction</option><option value="Cheque">Cheque</option><option value="Bad Debts">Bad Debts</option><option value="Salary Deduction">Salary Deduction</option><option value="Online Banking">Online Banking</option><option value="Red Cash">Red Cash</option></select></div></div> <div class="control-group"><label for="due_date" class="control-label">Due Date</label><div class="controls" style="margin-top:10px;"><span style="font-weight:bold;">14 Days</span><!- issue #7--></div></div>   <div class="control-group"><label class="control-label" for="deliver_type"><?php echo $this->lang->line("deliver_type"); ?></label><div class="controls"><select name="deliver_type" id="deliver_type"><option name="driver" value="driver">Driver</option><option name="teamleader" value="teamleader">Teamleader</option><option value="pickup" name="pickup">Pickup</option></select></div></div> <div id="pcash"><div class="control-group"><label class="control-label" for="paid-amount"><?php echo $this->lang->line("paid"); ?></label><div class="controls"><input type="text" id="paid-amount" /></div></div> <!-- issue #7 - Hyder -14 June 2014 - add order type - START --><div class="control-group"><label for="order_type" class="control-label">Order Type</label><div class="controls"><select id="order_type" name="order_type"><option name="direct" value="direct">Direct</option><option value="email" name="email">Email</option><option value="fax" name="fax">Fax</option><option name="phone" value="phone">Phone</option></select></div></div><!-- issue #7 - Hyder -14 June 2014 - add order type - END --><div class="control-group" style="font-weight:bold;"><div class="control-label" style="padding-top:0;font-weight:bold;"><?php echo $this->lang->line("change"); ?>:</div><div class="controls"><span style="background: #FFFF99; border-radius:5px; padding: 5px 10px; color: #000;" id="balance"></span></div></div></div> </div></div>');
    ...
    $("#paid_by").change(function () {
        ...
        if(p_val == 'cash' ||p_val == 'Credit' ) { // mne ajoute credit dans sa condition la - pu abdallah sa . - 14 June 2014 - Hyder  issue #36

            $('#pcash').show();
            $('input[id^="paid-amount"]').keydown(function(e){
                paid = $(this).val();
                if(e.keyCode == 13){
                    if($('#paid_by').val() == 'cash'){// mone ajoute sa pu ki kan met '0' pu payment type = credit, li laisse sales la passer - issue #36
                        if(paid < total) {
    ...
    // Issue  #7  - Hyder - First part

    $('#deliver_type').on('change', function() {

        $("#rdelivertype").val($("#deliver_type option:selected").text());
    });

    $('#order_type').on('change', function() {

        $("#rordertype").val($("#order_type option:selected").text());
    });



//$('#delivertype :selected').text();

//$('#delivertype').val()

//$('#delivertype').change(function(){
//               $.ajax({
//                    url:"index.php?module=pos&view",
//                    type:"POST",
//                    data:({}),
