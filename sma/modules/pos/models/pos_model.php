...
//die();
    ...
    /*  public function getSubCategories()
           {
            ...
            $this->db->select('subcategories.id,
                               ...
                               subcategories.code,
                               subcategories.name');
            $q = $this->db->get('subcategories')->where('subcategories.category_id', $category_id);
    ...
    public function fetch_menus($limit, $start)
    {
        $this->db->limit($limit, $start);
        $this->db->order_by("id", "asc");
    ...
    {
        $approved = 'approved';
        $q = $this->db->get_where('customers', array('id' => $id, 'status' => $approved), 1);
    ...
    public function getCustomerByTeamleader($id) // Hyder - 14 June 2014 - Issue #29
    {

        $q = $this->db->get_where('customers', array('teamleaderid' => $id), 1);
...
#############################################################################
            ...
            $kit_code = strtolower(substr($product_code, 0, 3));
            if ($kit_code == 'kit') { // si n item kit sa, b bizin treat li different pu kav decrease so stock
                $datax = $this->dechifrer_item_kit_details($items[$key]);
                foreach ($datax as $key => $itemsx) {
                    $product_id = isset($itemsx['product_id']) ? $itemsx['product_id'] : $itemsx['id'];
                    $product_quantity = isset($itemsx['quantity']) ? $itemsx['quantity'] * $data['quantity'] : $data['quantity']; // gift tou les temp so quantity c'est 1
...
###############################################################################
        ...
        $saleData = array(
            ...
            'due_date' => $saleDetails['due_date'], // issue #6 - Hyder - 8 July 2014
            'served_by' => USER_NAME, // issue #7- Hyder - 14 July 2014
            'order_type' => $saleDetails['order_type'], // issue #7 - Hyder - 14 July 2014
        );
...
############################################################
# Issue #1 - Hyder - N ban nouvo fonction sa - START       #
############################################################
    public function dechifrer_item_kit_details($items_arr)
    {
        $items = array();
        $data = $this->getAllItemkitItems($items_arr['product_itemkit_id']);
    ...
    public function getAllItemkitItems($itemkit_id)
    {
        ...
        if ($q->num_rows() > 0) {
            ...
            $data_gifts[] = $this->get_gift_products($itemkit_id); // rod ban cado la ek retourne zot product id

            $result = array_merge((array)$data, (array)$data_gifts[0]);

            return $result;
        }
    }

    public function get_gift_products($itemkit_id)
    {
        $q = $this->db->get_where('itemkits', array('id' => $itemkit_id));
        ...
        if ($q->num_rows() > 0) {
            ...
            $gifts[] = $record[0]->gift_one;
            $gifts[] = $record[0]->gift_two;
            $gifts[] = $record[0]->gift_three;
            $gifts[] = $record[0]->gift_four;
            $gifts[] = $record[0]->gift_five;
            $gifts = array_filter($gifts); // tir ban column ki null ou false..

            foreach ($gifts as $productid) {
                $data[] = (Array)$this->getProductByID($productid);
...
############################################################
# Issue #1 - Hyder - N ban nouvo fonction sa - END         #
############################################################
